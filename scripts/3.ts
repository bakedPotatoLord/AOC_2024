...
const allDos: [number, boolean, string][] = Array.from(raw.matchAll(/do\(\)/g))
  .map(({ 0: str, index: idx }) => [idx, false, str])
const allDonts: [number, boolean, string][] = Array.from(raw.matchAll(/don't\(\)/g))
  .map(({ 0: str, index: idx }) => [idx, false, str])

const arrindexed: [number, boolean, number][] = allReal.map(({ 0: str, index: idx }) => [idx, str.split(/,|\(|\)/)])
  .map(([i, str]) => [Number(i), true, parseInt(str[1]) * parseInt(str[2])])
